  ...
  <main>
    <div class="containerx-fluid">
        ...
        */
        ...
        if ($result0->num_rows > 0) 
        {while($row0 = $result0->fetch_assoc()) { 
            $stid=$row0["stid"]; $rollno=$row0["rollno"]; $prno=$row0["prno"];  $prdate=$row0["prdate"];   $amount=$row0["amount"];   $entrytime=$row0["entrytime"]; 
               $clsp=$row0["classname"];   $secp=$row0["sectionname"];
            ...
            ?>
            <div class="card text-center" style="background:var(<?php echo $bgc;?>); color:var(--darker);border-radius:0;" id="block<?php echo $prno;?>"  <?php echo $dsbl;?> >
                ...
                <table width="100%">
                    <tr>
                        ...
                        <td style="text-align:right; font-size:20px; font-weight:600;">
                            ...
                            if(strtotime($cur) - strtotime($entrytime)>$mini) {$ddd = 'disabled';} else {$ddd = '';}
