...
include 'inc.php';
	...
	$prno = $_POST['prno'];  
		    ...
		    $sql0r = "SELECT * FROM stfinance where sccode = '$sccode' and sessionyear='$sy' and  pr1no='$prno'  "; 
            $result0r = $conn->query($sql0r); if ($result0r->num_rows > 0) {while($row0r = $result0r->fetch_assoc()) { 
            $taka=$row0r["pr1"];  $id=$row0r["id"];
            ...
            $sql0r = "SELECT * FROM stfinance where sccode = '$sccode' and sessionyear='$sy' and  pr2no='$prno'  "; 
            $result0r = $conn->query($sql0r); if ($result0r->num_rows > 0) {while($row0r = $result0r->fetch_assoc()) { 
            $taka=$row0r["pr2"];  $id=$row0r["id"];
    		...
    		$query33 ="DELETE from stpr where prno='$prno' and sccode = '$sccode' and sessionyear='$sy';";//echo $query3g. '<br>';
    		$conn->query($query33); 
		    
		    
		    $sql0r = "SELECT * FROM sessioninfo where lastpr = '$prno'  "; 
            $result0r = $conn->query($sql0r); if ($result0r->num_rows > 0) {while($row0r = $result0r->fetch_assoc()) { 
            $sid=$row0r["id"];  $npr = $prno-1;
