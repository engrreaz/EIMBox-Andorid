...
$cur = date('Y-m-d H:i:s');; $sy = date('Y');
    ...
    */
    ...
    if($clsf == 'Six' || $clsf == 'Seven'){
        ...
        $result242a = $conn->query($sql242a); if ($result242a->num_rows > 0) {while($row242a = $result242a->fetch_assoc()) {
        $subcode=$row242a['subject']; $idr=$row242a['id']; 
	...
	/*
	$query3 ="insert into sessioninfo (id, stid, sessionyear, classname, sectionname, rollno, sccode) values 	(NULL, '$lastid','$sy','$cls','$sec','$x','$sccode')";
              $conn->query($query3) ;
                
                $query33 ="insert into students (id, stid, sccode) values (NULL, '$lastid','$sccode')";
	            $conn->query($query33);
	            $lastid = $lastid + 1;
	            */
	?>