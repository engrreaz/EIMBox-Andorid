<?php
    ...
    $result01xgr = $conn->query($sql0); if ($result01xgr->num_rows > 0) {while($row0 = $result01xgr->fetch_assoc()) { 
        $stid=$row0["stid"]; $rn=$row0["rollno"]; 

        $amto=$row0["taka"]; 
        
                $sql0 = "SELECT stnameeng FROM students where sccode='$sccode' and stid='$stid';";
                $result01xgrd = $conn->query($sql0); if ($result01xgrd->num_rows > 0) {while($row0 = $result01xgrd->fetch_assoc()) { 
                $nam=$row0["stnameeng"];$namb=$row0["stnameben"];}}
                if($nam == ''){$nam = $namb;}
        
        ?>

                    <div class="card " style="background:var(--lighter); color:black; border-radius:0; border:0; height:52px;"  >
                        <div style="width:10px; height:10px; left:20px; top:21px; position:absolute; background:deeppink; border-radius:50%;"></div>
