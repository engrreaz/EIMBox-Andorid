...
if ($clsf == 'Six' || $clsf == 'Seven') {
    ...
    $sql242a = "SELECT * FROM subjectsettinglist where classname='$clsf' and sectionname like '%$secx%' and sccategory = '$sctype' order by subject";
    $result242a = $conn->query($sql242a);
    if ($result242a->num_rows > 0) {
        while ($row242a = $result242a->fetch_assoc()) {
            $subcode = $row242a['subject'];
            ...
            $qq = "INSERT INTO subsetup (sessionyear, sccode, classname, sectionname, subject,fullmarks, subj, obj, pra, ca, camanual, pass_algorithm, cnt, reverse) SELECT '$YEAR', '$sccode', classname, '$secf', subject,fullmarks, subj, obj, pra, 0, camanual, pass_algorithm, cnt, reverse from subjectsettinglist where id = '$idr';";
            echo  $qq ;   $conn->query($qq);
