  ...
  <main>
    <div class="containerx-fluid">
        <div class="card text-left" style="background:var(--darker); color:var(--lighter); border-radius:0;" >
            <div class="card-body">
                <table width="100%" style="color:white;">
                    <tr>
                        <td colspan="2">
                            <div style="font-size:20px; text-align:center; padding: 2px 2px 8px; font-weight:700; line-height:15px;">Total Receivable
                            </div>
                        </td>
                    </tr>
                    <tr>
        ...
        <?php
            ...
            $result01x2 = $conn->query($sql0); if ($result01x2->num_rows > 0) {while($row0 = $result01x2->fetch_assoc()) { 
            $money=$row0["money"];  }}
            
            $sql0 = "SELECT sum(amount) as money2  FROM banktrans where transtype='Deposit' and entryby='$usr'"; 
            $result01x3 = $conn->query($sql0); if ($result01x3->num_rows > 0) {while($row0 = $result01x3->fetch_assoc()) { 
            $money2=$row0["money2"];  }}
            
            $sql0 = "SELECT sum(amount) as money3  FROM banktrans where transtype='Withdraw' and entryby='$usr'"; 
            $result01x4 = $conn->query($sql0); if ($result01x4->num_rows > 0) {while($row0 = $result01x4->fetch_assoc()) { 
            $money3=$row0["money3"];  }}
        ...
        <div class="card " style="background:var(--light); color:var(--dark); border-radius:0;"  onclick="gog('<?php echo $lnk;?>')">
              <img class="card-img-top"  alt="">
              <div class="card-body">
                <div style="font-size:30px; font-weight:700; ">
                    <span style="font-size:12px; font-weight:500;">BDT</span> <?php echo number_format($cimh,2,".",",");?>
                ...
                <div class="input-group mb-3"  id="exbox" style="display:flex">
                    ...
                    <select class="form-select form-select-md " id="partid2" aria-label=".form-select-lg example" style=" background:var(--lighter);">
                        <?php
                        $sql0 = "SELECT * FROM bankinfo where sccode='$sccode' and status=1 order by id";
                        $result0r = $conn->query($sql0);
                        if ($result0r->num_rows > 0) 
                        {while($row0 = $result0r->fetch_assoc()) { $accid = $row0["id"]; $bn = $row0["bankname"]; $type = $row0["acctype"]; 
                ...
                <div class="input-group mb-3">
                    ...
                    <input type="date" id="date" class="form-control" value="<?php echo date('Y-m-d');?>" placeholder="Description" aria-label="Description" aria-describedby="basic-addon1" style=" background:var(--lighter);">
                </div>
                ...
                <div class="input-group mb-3">
                    ...
                    <input type="number" id="amount" style="text-align:left;" class="form-control" value="<?php echo $cimh;?>" placeholder="Amount" aria-label="Description" aria-describedby="basic-addon1">
                </div> 
            ...
            <?php 
            $sl = 0; $sobar=0;
            $sql0 = "SELECT entryby, sum(amount) as mottaka FROM stpr where sccode='$sccode' and sessionyear='$sy'  group by entryby order by entryby ";
            //echo $sql0;
                $result0 = $conn->query($sql0);
                if ($result0->num_rows > 0) 
                {while($row0 = $result0->fetch_assoc()) { 
                    $by = $row0["entryby"];  $mottaka = $row0["mottaka"];  
                    ...
                    if($mottaka>0){
                ?>
            <div class="card " style="background:var(--lighter); color:var(--dark); border-radius:0;"  onclick="gog('<?php echo $lnk;?>')">
              <img class="card-img-top"  alt="">
              <div class="card-body">
                <div style="font-size:30px; font-weight:700; ">
                    <span style="font-size:12px; font-weight:500;">BDT</span> <?php echo number_format($mottaka,2,".",",");?>
                </div>
                <div style="font-size:11px; font-style:italic; color:gray; line-height:15px;">
                    <div id="from<?php echo $sl;?>"><?php echo $by;?></div>
                    <div style="font-size:14px; font-weight:600; color:var(--dark); font-style:normal;"><?php echo $tname;?></div>
                </div>
                
                <input id="cls<?php echo $sl;?>" value="<?php echo $ccc;?>" style="border:1px solid gray; border-radius: 4px; padding:5px; width:100px; margin-top:5px;" disabled/>
                <input id="sec<?php echo $sl;?>" value="<?php echo $sss;?>"   style="border:1px solid gray; border-radius: 4px; padding:5px; width:150px; margin-top:5px;;"   disabled/>
                
                <table style="width:100%; margin-top:5px;">
                    <tr>
                        <td style="width:50%">
                            <input style="text-align:left; font-size:20px;" id="amt<?php echo $sl;?>" type="text" class="input form-control" id="taka" value="<?php echo $mottaka;?>" />
                        </td>
                    ...
                    <tr>
                        <td>
                            <div id="btnbox<?php echo $sl;?>" class="mt-2"><button class="btn btn-warning btn-block"   onclick="pay(<?php echo $sl;?>)"> Received by Me</button></div>
                        </td>
                    </tr>
                </table>
            ...
            $result01xe = $conn->query($sql0); if ($result01xe->num_rows > 0) {while($row0 = $result01xe->fetch_assoc()) { 
                $eby=$row0["entryby"]; $iii=$row0["iii"]; $eee=$row0["eee"]; 
                
                $sql0 = "SELECT sum(amount) as paytaka FROM transaction where receivedfrom='$eby' and sccode='$sccode' and classname='Cashbook' ";
                    $result01xgrl = $conn->query($sql0); if ($result01xgrl->num_rows > 0) {while($row0 = $result01xgrl->fetch_assoc()) { $paytakas=$row0["paytaka"]; }}
              ...
              <div class="card-body">
                <div style="font-size:30px; font-weight:700; ">
                    <span style="font-size:12px; font-weight:500;">BDT</span> <?php echo number_format($bbb,2,".",",");?>
                </div>
                <div style="font-size:11px; font-style:italic; color:gray; line-height:15px;">
                ...
                <table style="width:100%; margin-top:5px;">
                    <tr>
                        <td style="width:50%">
                            <input style="text-align:left; onchange="db(<?php echo $num;?>);" font-size:20px;" id="amtt<?php echo $num;?>" type="text" class="input form-control" id="taka" value="<?php echo $bbb;?>" />
                        </td>
                    ...
                    <tr>
                        <td>
                            <div id="btnbox2<?php echo $num;?>" class="mt-2"><button class="btn btn-info btn-block"   onclick="pay2('<?php echo $num;?>')"> Received by Me</button></div>
                        </td>
                    </tr>
                </table>
